...
LOG_FILE = '/home/ross/pi-smart-display/network-monitor.log'
...
class NetworkMonitor:
    def __init__(self):
        ...
        logging.info("Network monitor started - Checking connectivity every %d seconds", CHECK_INTERVAL)
        logging.info("Will restart network service after %d consecutive failures", self.max_failures)
    ...
    def restart_network_service(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    logging.info("Network service restarted successfully via service command")
                    return True
                else:
            ...
            while self.running:
                if self.check_connectivity():
                    if self.consecutive_failures > 0:
                        logging.info("Connectivity restored after %d failures", self.consecutive_failures)
                    self.consecutive_failures = 0
                else:
                    self.consecutive_failures += 1
                    logging.warning("Connectivity check failed (%d/%d)", self.consecutive_failures, self.max_failures)
                    
                    if self.consecutive_failures >= self.max_failures:
                        logging.error("Max failures reached, restarting network service...")
